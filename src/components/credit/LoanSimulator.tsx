    ...
    <div className="space-y-6">
      ...
      {calculations && (
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Cumulative Interest Paid</CardTitle>
              <CardDescription>
                How interest accumulates over the life of the loan
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Tooltip 
                      formatter={(value: number, name: string) => [
                        formatCurrencySimple(value),
                        name === 'baseInterest' ? 'Standard Loan Interest' : 'Extra Payment Interest'
                      ]}
                      labelFormatter={(month: number) => `Month ${month}`}
                    />
                    <Area 
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {calculations.extraPaymentResults 
                      ? 'Payment schedule with extra principal payments'
              ...
              <CardContent>
                <div className="max-h-96 overflow-y-auto">
                  <Table>
                    ...
                    <TableBody>
                      {(calculations.extraPaymentResults?.amortizationSchedule || calculations.amortizationSchedule)
                        .slice(0, 60)
                        .map((entry, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>{formatCurrencySimple(entry.balance)}</TableCell>
                          <TableCell className="text-muted-foreground">
                            {formatCurrencySimple(entry.cumulativeInterest)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  ...
                  <p className="text-sm text-muted-foreground text-center mt-4">
                    Showing first 60 payments of {
                      (calculations.extraPaymentResults?.amortizationSchedule || calculations.amortizationSchedule).length
                    } total payments
                  </p>
                </div>
              </CardContent>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    ...
                    <li>• Interest rate: {annualRate}% annually = {(annualRate/12).toFixed(2)}% monthly</li>
                    <li>• Total interest is {((calculations.totalInterest / loanAmount) * 100).toFixed(0)}% of loan amount</li>
                    {calculations.extraPaymentResults && (
                      <li className="text-financial font-medium">
                        • Extra ${formatCurrencySimple(extraPayment)}/month saves ${formatCurrencySimple(calculations.extraPaymentResults.interestSaved)} in interest!
                      </li>
                    )}
                  </ul>
                </div>
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    <li>• Any extra payment goes directly to principal</li>
                    <li>• Bi-weekly payments (26/year) = 13 monthly payments annually</li>
                    <li>• Even $25-50 extra monthly can save thousands in interest</li>
                    <li>• Pay off highest interest rate debts first</li>
                  </ul>
                </div>
              </div>
            </CardContent>
